...
# PATCH: Override SDK paths for macOS Sonoma + Xcode 16 environment
# This must be done BEFORE loading mkmf so it picks up the changed config
if RbConfig::CONFIG['host_os'] =~ /darwin/
  sdk_path = "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/System/Library/Frameworks/Ruby.framework/Versions/2.6/usr/include/ruby-2.6.0"
  if File.directory?(sdk_path)
    puts "Patching RbConfig for SDK path: #{sdk_path}"
    RbConfig::CONFIG['rubyhdrdir'] = sdk_path
    RbConfig::CONFIG['rubyarchhdrdir'] = "#{sdk_path}/universal-darwin24"
    RbConfig::CONFIG['arch'] = 'universal-darwin24'
    RbConfig::CONFIG['sitearch'] = 'universal-darwin24'
    # We also need to help mkmf find it by adding to flags explicitly immediately
    # environment variables might be better but let's try modifying CONFIG['CFLAGS'] if possible or just rely on arch update
  end
end
...
if RUBY_ENGINE == 'ruby' || RUBY_ENGINE == 'rbx'
  ...
  if RbConfig::CONFIG['host_os'] =~ /darwin/
      $INCFLAGS << " -I#{RbConfig::CONFIG['rubyarchhdrdir']}"
  end
